            ...
            if (order.items.isEmpty)
              Center(
                child: Padding(
                  ...
                  child: Text(order.orderDetails ?? l10n.noAvailableOrders),
                ),
              )
            else
              ListView.separated(
  ...
  Widget _buildInfoCard(BuildContext context) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            _buildInfoRow(LucideIcons.store, order.restaurant?.name ?? ''),
            const SizedBox(height: 8),
            _buildInfoRow(LucideIcons.mapPin, order.restaurant?.address ?? ''),
            const Divider(height: 24),
            _buildInfoRow(LucideIcons.user, order.customerName),
            const SizedBox(height: 8),
            _buildInfoRow(LucideIcons.navigation, order.customerAddress),
          ],
        ),
      ),
    );
